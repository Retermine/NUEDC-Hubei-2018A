...
{
	Xil_Out32(XPAR_TFTCONTROLLER_0_S00_AXI_BASEADDR + TFTCONTROLLER_S00_AXI_SLV_REG0_OFFSET, reg + data); //write reg
}
...
{
	Xil_Out32(XPAR_TFTCONTROLLER_0_S00_AXI_BASEADDR + TFTCONTROLLER_S00_AXI_SLV_REG0_OFFSET, is_data + data); //write data
}
//读LCD数据
//返回值:读到的值
...
void LCD_DrawExcel(u16 x1,u16 y1,u16 x2,u16 y2,u16 xdiv,u16 ydiv,u16 style)
{
	POINT_COLOR = WHITE;
	LCD_DrawRectangle(x1,y1,x2,y2);
	short i,n,width,height,posx,posy;
	width = (float)(x2-x1)/xdiv;
	height = (float)(y2-y1)/ydiv;
	if(style)
	{
//		POINT_COLOR = GRAY;
//		for(n=1;n<xdiv;n++)LCD_DrawLine(x1+width*n,y1,x1+width*n,y2);	//画列
//		for(n=1;n<ydiv;n++)LCD_DrawLine(x1,y1+height*n,x2,y1+height*n);		//画行
		for(n=1;n<xdiv;n++)
		{
			LCD_ShowNum(x1+width*n-25,440,n*5000/xdiv,4,24);
		}
	}else{
		for(n=1;n<xdiv;n++)
		{
			LCD_ShowNum(x1+width*n-25,440,n*1000/xdiv,4,24);
		}
	}
